...
const client = env.openaiApiKey ? new OpenAI({ apiKey: env.openaiApiKey, timeout: 60_000, maxRetries: 1 }) : null;
...
export async function judgeMonitor(condition: string, messages: JudgeMessage[]): Promise<JudgeResult> {
  ...
  try {
    ...
    if (start < 0 || end < 0) return { matched: false, summary: "" };
    const json = JSON.parse(text.slice(start, end + 1));
    return { matched: Boolean(json.matched), summary: String(json.summary || "").slice(0, 600) };
  } catch (error) {
